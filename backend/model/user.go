...
type User struct {
	...
	Status        uint        `json:"status" gorm:"column:status;type:tinyint(3) unsigned;default:1;NOT NULL;comment:1-可用,2-不可用,3-未激活"`
	Mobile        string      `json:"mobile" gorm:"column:mobile;type:char(11);comment:手机号"`
	IconPath      string      `json:"iconPath" gorm:"column:icon_path;type:varchar(100);comment:头像地址"`
