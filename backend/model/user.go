...
type User struct {
	...
	TokenExpire   *LocalTime `gorm:"column:token_expire;type:datetime;comment:' token超时时间'"`
	LastLoginIp   string     `gorm:"column:last_login_ip;type:varchar(100);comment:' 最后登陆ip地址'"`
	LastLoginTime *LocalTime `gorm:"column:last_login_time;type:datetime;comment:' 最后登陆时间'"`
}
